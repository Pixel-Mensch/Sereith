...
2. Verify the live Ollama path with a running local model from the desktop and CLI paths.
Status: In progress
...
Notes: Current verified coverage includes eight focused engine tests and startup smoke checks run manually.
...
Notes: Module maps still reflect the earlier scaffold and do not yet cover the new desktop path.
